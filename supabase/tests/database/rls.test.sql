...
-- Enable pgTAP extension before calling plan()
...
-- Grant execution on pgTAP test assertion functions in schema public to test roles
...
select throws_ok(
  ...
  '42501',
  null,
  '1. anon role cannot select from user_preferences'
);
...
select throws_ok(
  ...
  '42501',
  null,
  '2. anon role cannot select from content_pillars'
);
...
select throws_ok(
  ...
  '42501',
  null,
  '3. anon role cannot select from content_items'
);
...
select throws_ok(
  ...
  '42501',
  null,
  '4. anon role cannot select from content_media'
);
...
select throws_ok(
  ...
  '42501',
  null,
  '5. anon role cannot insert into content_items'
);
...
select lives_ok(
  ...
  '6. User 1 can insert own user_preferences'
);
...
select lives_ok(
  ...
  '7. User 1 can create own content pillar'
);
...
select lives_ok(
  ...
  '8. User 1 can create content item with own pillar'
);
...
select lives_ok(
  ...
  '9. User 1 can insert media for own item'
);
...
select results_eq(
  ...
  $$ values (1) $$,
  '10. User 1 sees exactly their own content item'
);
...
select is_empty(
  ...
  '11. User 2 cannot read User 1 content items'
);
...
select lives_ok(
  ...
  '14. User 2 update statement executes cleanly without modifying User 1 rows'
);

select is(
  (select title from public.content_items where id = 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb'),
  null,
  '15. User 2 cannot see or alter User 1 item title'
);
...
select throws_ok(
  ...
  null,
  null,
  '16. User 2 cannot link item to User 1 content pillar on INSERT'
);
...
select throws_ok(
  ...
  null,
  null,
  '17. User 2 cannot update item to reference User 1 content pillar'
);
...
select throws_ok(
  ...
  null,
  null,
  '18. User 2 cannot insert media referencing User 1 content item'
);
...
select throws_ok(
  ...
  null,
  null,
  '19. User 2 cannot update media to reference User 1 content item'
);
...
select lives_ok(
  ...
  '20. User 1 deleting referenced pillar succeeds'
);
...
select results_eq(
  ...
  $$ values (1) $$,
  '21. Content item is preserved after pillar deletion'
);
...
select results_eq(
  ...
  '22. Content item owner user_id is preserved and not null'
);

select is(
  (select content_pillar_id from public.content_items where id = 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb'),
  null,
  '23. Content item content_pillar_id is set to null after pillar deletion'
);
...
select lives_ok(
  ...
  '24. User 1 can upload object into own folder in content-media bucket'
);
...
select throws_ok(
  ...
  null,
  null,
  '25. User 2 cannot upload object into User 1 folder'
);
...
select is_empty(
  ...
  '26. User 2 cannot select User 1 storage objects'
);
